-- Create the database
CREATE DATABASE IF NOT EXISTS project_checkin;

-- Use the created database
USE project_checkin;

-- Table for teams
CREATE TABLE teams (
    team_id INT AUTO_INCREMENT PRIMARY KEY,
    team_name VARCHAR(255) NOT NULL,
    team_code VARCHAR(10) UNIQUE NOT NULL
);

-- Table for team members
CREATE TABLE team_members (
    member_id INT AUTO_INCREMENT PRIMARY KEY,
    team_id INT,
    first_name VARCHAR(50) NOT NULL,
    last_name VARCHAR(50) NOT NULL,
    FOREIGN KEY (team_id) REFERENCES teams(team_id)
);

-- Table for check-in questions
CREATE TABLE checkin_questions (
    question_id INT AUTO_INCREMENT PRIMARY KEY,
    team_id INT,
    question_text VARCHAR(255) NOT NULL,
    frequency ENUM('daily', 'weekly', 'biweekly', 'monthly') NOT NULL,
    next_due_date TIMESTAMP,
    FOREIGN KEY (team_id) REFERENCES teams(team_id)
);

-- Table for responses to check-in questions
CREATE TABLE checkin_responses (
    response_id INT AUTO_INCREMENT PRIMARY KEY,
    question_id INT,
    member_id INT,
    response_text TEXT,
    response_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (question_id) REFERENCES checkin_questions(question_id),
    FOREIGN KEY (member_id) REFERENCES team_members(member_id)
);

-- Insert sample data for teams
INSERT INTO teams (team_name, team_code) VALUES
('Team A', 'ABC123'),
('Team B', 'DEF456'),
('Team C', 'GHI789');

-- Insert sample data for team members
INSERT INTO team_members (team_id, first_name, last_name) VALUES
(1, 'John', 'Doe'),
(1, 'Jane', 'Smith'),
(2, 'Alice', 'Johnson'),
(2, 'Bob', 'Williams'),
(3, 'Emily', 'Brown');

-- Insert sample data for check-in questions
INSERT INTO checkin_questions (team_id, question_text, frequency, next_due_date) VALUES
(1, 'How do you feel today?', 'daily', NOW()),
(2, 'How did it go yesterday?', 'daily', NOW() + INTERVAL 1 DAY),
(3, 'What will you do today?', 'daily', NOW() + INTERVAL 2 DAYS),
(4, 'Are you stuck on something?', 'weekly', NOW()),
(5, 'Would you like to pair on something?', 'weekly', NOW() + INTERVAL 1 WEEK);

-- Insert sample data for check-in responses
INSERT INTO checkin_responses (question_id, member_id, response_text) VALUES
(1, 1, 'Feeling good!'),
(1, 2, 'Could be better.'),
(2, 3, 'Had a productive day yesterday.'),
(4, 5, 'Yes, I need help with a bug.'),
(5, 5, 'Sure, let''s pair on the new feature.');
